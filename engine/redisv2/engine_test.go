...
func TestEngine_Consume2(t *testing.T) {
	...
	jobID2, err := e.Publish("ns-engine", "q3", []byte("delay msg"), 10, 8, 1)
	if err != nil {
		t.Fatalf("Failed to publish job1: %s", err)
	}
	t.Log(jobID2)
	jobID1, err := e.Publish("ns-engine", "q3", body, 10, 3, 1)
	if err != nil {
		t.Fatalf("Failed to publish job2: %s", err)
	}
	t.Log(jobID1)
