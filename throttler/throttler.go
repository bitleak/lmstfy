...
func (t *Throttler) RemedyLimiter(pool, namespace, token string, isRead bool) error {
	...
	_, err := t.redisCli.EvalSha(ctx, t.decrSHA, []string{tokenCounterKey}).Result()
	if err != nil && strings.HasPrefix(err.Error(), "NOSCRIPT") {
		sha, err := t.redisCli.ScriptLoad(ctx, throttleDecrLuaScript).Result()
		if err != nil {
			return err
		}
		t.decrSHA = sha
...
func (t *Throttler) IsReachRateLimit(pool, namespace, token string, isRead bool) (bool, error) {
	...
	tokenCounterKey := t.buildCounterKey(pool, namespace, token, isRead)
	val, err := t.redisCli.EvalSha(ctx, t.incrSHA, []string{tokenCounterKey}, limiter.Interval).Result()
	if err != nil && strings.HasPrefix(err.Error(), "NOSCRIPT") {
		sha, err := t.redisCli.ScriptLoad(ctx, throttleIncrLuaScript).Result()
		if err != nil {
...
func (t *Throttler) Add(pool, namespace, token string, limiter *Limiter) error {
	...
	ok, err := t.redisCli.HSetNX(ctx, throttlerRedisKey, tokenLimitKey, string(bytes)).Result()
	if err != nil {
...
func (t *Throttler) Set(pool, namespace, token string, limiter *Limiter) error {
	...
	tokenLimitKey := t.buildLimitKey(pool, namespace, token)
	_, err = t.redisCli.HSet(ctx, throttlerRedisKey, tokenLimitKey, string(bytes)).Result()
	if err != nil {
...
func (t *Throttler) Delete(pool, namespace, token string) error {
	tokenLimitKey := t.buildLimitKey(pool, namespace, token)
	_, err := t.redisCli.HDel(ctx, throttlerRedisKey, tokenLimitKey).Result()
	if err != nil {
...
func (t *Throttler) updateLimiters() {
	...
	results, err := t.redisCli.HGetAll(ctx, throttlerRedisKey).Result()
	if err != nil {
...
func Setup(conf *config.RedisConf, logger *logrus.Logger) error {
	...
	sha1, err := cli.ScriptLoad(ctx, throttleIncrLuaScript).Result()
	if err != nil {
		return fmt.Errorf("load the throttle incr script: %s", err.Error())
	}
	sha2, err := cli.ScriptLoad(ctx, throttleDecrLuaScript).Result()
	if err != nil {
