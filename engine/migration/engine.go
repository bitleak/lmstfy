...
func (e *Engine) Publish(namespace, queue string, body []byte, ttlSecond, delaySecond uint32, tries uint16, priority uint8) (jobID string, err error) {
	return e.newEngine.Publish(namespace, queue, body, ttlSecond, delaySecond, tries, 0)
}
...
func (e *Engine) ConsumeMultiWithFrozenTries(namespace string, queues []string, ttrSecond, timeoutSecond uint32) (job engine.Job, err error) {
	job, err = e.oldEngine.ConsumeMultiWithFrozenTries(namespace, queues, ttrSecond, 1)
	if job != nil {
		return // During migration, we always prefer the old engine's data as we need to drain it
	}
	return e.newEngine.ConsumeMultiWithFrozenTries(namespace, queues, ttrSecond, timeoutSecond)
}
